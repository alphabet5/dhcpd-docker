...
Currently only Cisco IOS is supported. 
    ...
    parser.add_argument('--configure', help="Configure the interfaces on the list of switches for inserting the circuit-id.", default=False, action='store_true')
    parser.add_argument('--export-clients', help="TODO: Export a list of circuit-id's and ip's for use in the DHCP server.", default=False, action='store_true')
    parser.add_argument('--sw-user', help="Optional: Switch Username. Can also be added by ENV SW_USER", type=str, default='')
    parser.add_argument('--sw-pass', help="Optional: Switch Password. Can also be added by ENV SW_PASS", type=str, default='')
    parser.add_argument('--sw-list', help="Optional: Switch list /directory/filename.txt. Defaults to /switch_list.txt.", type=str, default='/switch_list.txt')
...
"""
                ...
                hostname = parse.find_objects(r"^hostname")[0].text.replace("hostname ", "")
                ...
                for interface in interfaces_raw:
                    if "vlan" not in interface.text.lower():
                        if " switchport mode access" in [c.text for c in interface.children]:
                            snooping_vlans = list()
                            for child in interface.children:
                                if "switchport access vlan" in child.text:
                                    vlan = re.match(r".*vlan (\d+).*", child.text).group(1)
                        ...
                        interfaces.append(
                            canonical_interface_name(interface.text.replace("interface ", ""))
                        )
...
"""
                ...
                accept = console.input("Accept config merge for " + switch + "? (y/n) [n]")
